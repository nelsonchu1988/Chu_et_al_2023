function map_coord_individual(indexes, file_name, z_coords, output_folder, data_folder)
    ...
    for i = 1:length(subs)
        sub = subs{i};
        resultFile = [data_folder filesep sub '_dist_' dim '_' num2str(r(1)) '.mat'];
        data{i} = load(resultFile);
        counter_per_subs(i) = length(data{i}.distances);
        dataCXCL12{i} = readmatrix([data_folder filesep 'YPOP' filesep sub filesep sub '_CXCL12_coordinates_crop.csv']);
    end
    ...
    writetable(T, [output_folder filesep file_name '.xlsx']);
end